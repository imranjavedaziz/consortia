...
const CompleteFileUpload = ({
  ...
  const myBucket = new AWS.S3({
    params: {
      Bucket: privateBucket
        ? process.env.REACT_APP_UNLOCKABLE_BUCKET_NAME
  ...
  return (
    <div className="space-y-1.5">
      <h4 className="text-label-xs sm:text-label-sm xl:text-label-lg leading-4 sm:leading-[1.4375em] text-white font-normal">
        {labelName}
      </h4>
      ...
      <div
        className={`p-0.5 border-[#b731ff] ${borderRadius} border-[2px] border-dashed rounded-[24px] cursor-pointer`}
      >
        <label htmlFor={`fileInput-${formId}`} className="cursor-pointer">
          <div
            ...
            className={`w-full h-[150px] {${borderRadius} flex justify-center items-center bg  px-2 overflow-hidden rounded-[24px] `}
            onDrop={handleDrop}
            ...
            {file == "" ? (
              <>
                <img
                  ...
                  alt=""
                />
                <span className="text-[8px] sm:text-[12px] text-[#fafbfc] opacity-50 ml-2">
